...
/**
 * To add a new style to Responsive Menus, you are likely creating at least.
 *
 * 3 functions:
...
function hook_responsive_menus_style_info() {
  ...
  $styles = [
    'example_style' => [
      ...
      'css_files' => [$path . '/css/example.css'],
 ...
 *   js_settings: string - Function name to build JS settings
 *   passed to drupal_add_js().
 ...
 *   selector: string - Text for the admin page describing which selector to
 *   use.
 *   file: string - Optional file with the form & js_settings
 *   callback functions.
 *
 *   Other notes:
 ...
 *   First, showing all the available options.  Note including
 *   js_files, css_files
 *   AND specifying use_libraries => TRUE would result in the Libraries module
 *   first trying to load the library, then Responsive
 *   Menus would drupal_add_[type]
 ...
 */
function hook_responsive_menus_styles_alter(array &$styles) {
  ...
  $styles['my_style'] = [
    ...
    'css_files' => [$path . '/my_style/my_style.css'],
  ...
  $styles['sidr'] = [
    ...
    'css_files' => [$path . '/sidr/my_sidr.css'],
  ];
}
